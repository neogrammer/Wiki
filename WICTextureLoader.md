...
> On Windows 8.1 and Windows 10, you can load simple 2D textures in BC1, BC2, or BC3 pixel format ``DDS`` files using WIC as there is a basic DDS built-in codec present. All other formats and resource types will fail, and you'll get ``WINCODEC_ERR_COMPONENTNOTFOUND`` on older versions of Windows trying to use WICTextureLoader to load ``DDS`` files. Use [[DDSTextureLoader]] instead.
