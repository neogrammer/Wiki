...
If you want to give a pointer in one ComPtr to another ComPtr without changing the reference count (i.e. transferring ownership), you need to use ``Detach`` from the old ComPtr followed by an ``Attach`` to the new one. Alternatively, you can generally use ``Swap`` to achieve the same thing when one of the ComPtr variables involved is already null.
