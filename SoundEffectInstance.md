...
Note if the instance was created without _``SoundEffectInstance_Use3D``_, then calls to **Apply3D** will result in a C++ exception being thrown. **Apply3D** will overwrite any ``SetPan`` settings.
