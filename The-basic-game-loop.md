...
> There are two versions of each D3DGame template. The "DR" version makes use of a [DeviceResources](http://blogs.msdn.com/b/chuckw/archive/2015/12/17/direct3d-game-visual-studio-templates-redux.aspx) abstraction to isolate the device and swapchain handling into a helper file. This is useful for larger projects, but for these lessons I'm making use of the non-DR version which has all the Direct3D objects inside the ``Game`` class.
