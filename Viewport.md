...
* **ComputeTitleSafeArea**: When displaying on televisions, edges of the screen can't always be seen by users. This function takes a backbuffer width and height in pixels and returns a [safe area](https://en.wikipedia.org/wiki/Safe_area_%28television%29) rectangle for the content to be displayed. This function uses the SMPTE ST 2046-1 standard of 10% safe area intended for HDTVs, rather than the older SMPTE RP 8 intended for CRTs that used 20% safe area which is what the original XNA Game Studio math function implemented.
